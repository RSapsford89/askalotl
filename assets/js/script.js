document.addEventListener('DOMContentLoaded', () => {
    ...
    function animalSelection(animal) {
        ...
        const whaleText = `Blue whales are the biggest animals on
        ...
        const axolotlText = `Axolotls are magical little creatures that live underwater and can
        ...
        const penguinText = `Penguins are waddle-tastic birds that love the cold! They
        ...
        const catText = `Cats are playful, curious, and sometimes a little sneaky!
    ...
    function nextFact(animal) {
        ...
        gsap.set('#introText', { boxShadow: 'none', opacity: 1 });
        const tl = gsap.timeline();
        tl.from('#introTitle', {
            direction: 'left', duration: 0.8, opacity: 0, x: -100,
        })
            .from('#introText', { direction: 'left', duration: 1, opacity: 0 }, '-=0.5');
    ...
    function showHideMenu() {
        ...
        gsap.set('#restartBtn', { opacity: 1, y: 0 });
        gsap.set('#videoBtn', { opacity: 1, y: 0 });
        gsap.set('#quizBtn', { opacity: 1, y: 0 });
        gsap.set('#galleryBtn', { opacity: 1, y: 0 });
        // if the menu bool is false (menu not open)
        if (openMenu === false) {
            // animate the buttons nicely upwards 'from' move from the defined pos to the CSS pos
            const tl = gsap.timeline();
            tl.from('#restartBtn', { duration: 0.5, opacity: 1, y: 55 })
                .from('#videoBtn', { duration: 0.5, opacity: 1, y: 110 }, '-=0.2')
                .from('#quizBtn', { duration: 0.5, opacity: 1, y: 165 }, '-=0.3')
                .from('#galleryBtn', { duration: 0.5, opacity: 1, y: 220 }, '-=0.4');
            ...
            tl.to('#restartBtn', { duration: 0.5, opacity: 1, y: 55 })
                .to('#videoBtn', { duration: 0.5, opacity: 1, y: 110 }, '-=0.2')
                .to('#quizBtn', { duration: 0.5, opacity: 1, y: 165 }, '-=0.3')
                .to('#galleryBtn', { duration: 0.5, opacity: 1, y: 220 }, '-=0.4');
    ...
    function nameValidation() {
        ...
        const re = /[^A-Z-a-z]/g;// regular expression. Negated set - (true if does not include A-z)
    ...
    function quizContents() {
        ...
        switch (selectedAnimal) {
            ...
            case 'cat':
                ...
                break;
            default:
                console.log(`${quizContents} not an accepted case. quizContents() accepts whale, penguin, cat, axolotl`);
                break;
        }
    }
    ...
    function endQuiz(quizScore) {
        ...
        resultP.innerText = `You scored ${quizScore} out of 3. Try the quiz again for different questions.`;
    }
    ...
    function imgAreaScaler(imgId) {
        ...
        const currentX = img.width;// live image size in viewport - changes due to responsiveness
        const ratio = currentX / previousSize;// to yield ratio -> newsize/oldsize
    ...
    function imgMapScaler(ratio) {
        const whale = document.querySelector('area[data-attribute="whale"]');
        const axolotl = document.querySelector('area[data-attribute="axolotl"]');
        const penguin = document.querySelector('area[data-attribute="penguin"]');
        const cat = document.querySelector('area[data-attribute="cat"]');

        const strWhale = (whale.getAttribute('coords')).split(',');
        const strAxolotl = (axolotl.getAttribute('coords')).split(',');
        const strPenguin = (penguin.getAttribute('coords')).split(',');
        const strCat = (cat.getAttribute('coords')).split(',');

        const numWhale = strWhale.map(Number);
        const numAxolotl = strAxolotl.map(Number);
        const numPenguin = strPenguin.map(Number);
        const numCat = strCat.map(Number);

        const newArr = [16];
        for (i = 0; i < numWhale.length; i++) {
    ...
    const split = SplitText.create('#headingTitle', { type: 'chars' });
